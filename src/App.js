...
export default function App() {
  ...
  /** ---- Fetch single invoice based on URL ---- */
  useEffect(() => {
  ...
  const handleApprove = async (invoice) => {
    try {
      ...
      // Update status
      await supabase.from("backend").update({ status: "APPROVED", total, amount: total }).eq("phonenumber", invoice.phonenumber);
      ...
      setInvoice({ ...invoice, status: "APPROVED", pdf_url: pdfUrl });
  ...
  return (
    <div style={{ padding: 20, opacity: loading ? 0.6 : 1 }}>
      ...
      <table border="1" cellPadding="6" style={{ width:"100%", borderCollapse:"collapse" }}>
        <thead>
          <tr>
            <th>Product</th><th>Description</th><th>Qty</th><th>Units</th><th>Rate</th><th>Amount</th>{editId && <th>Action</th>}
          </tr>
        </thead>
        <tbody>
          {rows.map((r,i)=>{
          ...
          <tr>
            ...
            <td style={{fontWeight:"bold"}}>{editId ? calcEditTotals.total.toFixed(2) : total.toFixed(2)}</td>
            {editId && <td />}
          </tr>
        </tbody>
      </table>

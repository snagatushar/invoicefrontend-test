...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJlZ2ZqeGx2amF1Ym5pemt2cnV3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTYwNjM0MzcsImV4cCI6MjA3MTYzOTQzN30.P6s1vWqAhXaNclfQw1NQ8Sj974uQJxAmoYG9mPvpKSQ"; // <-- Replace with correct anon key
...
const generatePDFBlob = (invoiceLike) => {
  ...
  const tableData = rows.map((r) => {
    ...
    total += amount;
    return [r.productname, r.description, r.quantity, r.units, r.rate, amount.toFixed(2)];
  });
...
function UserInvoices() {
  ...
  // Approve invoice & trigger webhook
  const handleApprove = async (invoice) => {
    try {
      ...
      await fetch("https://n8n-image2doc-u35379.vm.elestio.app/webhook-test/f06adee0-b5f2-40f4-a293-4ec1067a14b0", {
        ...
        body: JSON.stringify({ invoice_number: invoice.invoice_number, phonenumber: invoice.phonenumber, pdf_url: pdfUrl, status: "APPROVED" }),
      });

      alert("✅ Approved, PDF uploaded & webhook triggered!");
      fetchData();
    } catch (err) { console.error(err); alert("❌ Approval failed"); }
    finally { setLoading(false); }
  };

  // Edit functions
  const handleEdit = (inv) => setEditData({ ...inv, rows: normalizeRows(inv) }) || setEditId(inv.invoice_number);
  const handleChangeHeader = (field, value) => setEditData(s => ({ ...s, [field]: value ?? "" }));
  const handleRowChange = (index, field, value) => setEditData(s => ({ ...s, rows: s.rows.map((r,i)=>i===index?{...r,[field]:value}:r) }));
  ...
  const calcEditTotals = useMemo(()=>{ 
    if(!editData?.rows) return {total:0};
    return {total: editData.rows.map(r=>num(r.quantity)*num(r.rate)).reduce((a,b)=>a+b,0)};
  }, [editData]);
  ...
  const handleSave = async () => {
    try {
      ...
      const { error } = await supabase.from("backend")
        .update(payload)
      ...
      {invoices.map(inv=>{
        ...
        return (
          <div key={inv.invoice_number} style={{border:"2px solid #222",marginBottom:20,padding:12,borderRadius:8}}>
            <h3>INVOICE: {isEditing?<input value={editData.invoice_number} onChange={e=>handleChangeHeader("invoice_number",e.target.value)}/>:inv.invoice_number}</h3>
            {isEditing?
              ...
              : <p><b>DEALER:</b> {inv.Dealer}<br/><b>PHONE:</b> {inv.phonenumber}<br/><b>DATE:</b> {inv.invoice_date}<br/><b>STATUS:</b> {inv.status}<br/>{inv.pdf_url && <a href={inv.pdf_url} target="_blank" rel="noreferrer">📄 View PDF</a>}</p>
            }
            <table border="1" cellPadding="6" style={{marginTop:10,width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>
                <th>PRODUCT</th><th>DESCRIPTION</th><th>QUANTITY</th><th>UNITS</th><th>RATE</th><th>AMOUNT</th>{isEditing&&<th>ACTION</th>}
              </tr></thead>
              <tbody>

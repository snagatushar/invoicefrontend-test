...
function UserInvoices() {
  ...
  const fetchData = async () => {
    ...
    if (error) { console.error(error); alert("Failed to load invoices."); return; }
  ...
  const handleRowChange = (index, field, value) => { setEditData((s) => { const rows = [...s.rows]; rows[index] = { ...rows[index], [field]: value ?? "" }; return { ...s, rows }; }); };
  ...
  const handleSave = async () => {
    try {
      ...
      const { error } = await supabase.from("backend").update(payload).eq("invoice_number", editId).eq("phonenumber", phonenumber);
        ...
        return (
          <div key={inv.invoice_number} style={{ border: "2px solid #222", marginBottom: 20, padding: 12, borderRadius: 8 }}>
            ...
            <table border="1" cellPadding="6" style={{ marginTop: 10, width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr><th>PRODUCT</th><th>DESCRIPTION</th><th>QUANTITY</th><th>UNITS</th><th>RATE</th><th>AMOUNT</th>{isEditing && <th>ACTION</th>}</tr>
              </thead>
              <tbody>
